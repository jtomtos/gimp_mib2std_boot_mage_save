...
def fileMIB2save(image, drawable, filename, raw_filename, dummy, limcolors, extlbl):
    ...
    height = image.height
    width = image.width
    if (image.width == lbl_width and image.height==lbl_height):
        extlbl = False
    
    #prepare source image
    img = pdb.gimp_image_duplicate(image)    
    try:
        #reduce colors to 64 to fit 1mb limit       
    ...
    for i in xrange(0, width):
        for j in xrange(0, height):
            ...
            if ((i % 2) == 0):
                r = (r + src_pixels[src_pos + sp_size: src_pos + sp_size + sp_size][0])/2
                px = (((b - r)/2 + 128) + ((b - g)/2 + 128))/2   
            else:
                b = (b + src_pixels[src_pos - sp_size: src_pos][2])/2
...
register(
    ...
    [
        ...
        (PF_INT, "lim-colors", _("Limit colors to fit size limit"), 128),
